...
-- Swap model for growth stages
local function SwapPlantModel(plantId, modelName)
    local crop = Crops[plantId]
    if not crop then
        DBG.Error('SwapPlantModel called for non-existent plant: ' .. tostring(plantId))
        return
    end

    if not modelName or modelName == '' then
        DBG.Warning('SwapPlantModel called with empty modelName for plant: ' .. tostring(plantId))
        return
    end

    local hash = joaat(modelName)
    if not LoadModel(hash, modelName) then
        DBG.Error('Failed to load stage model: ' .. modelName)
        return
    end

    if not crop.object or not DoesEntityExist(crop.object) then
        DBG.Error('SwapPlantModel: original plant object does not exist for plant: ' .. tostring(plantId))
        return
    end

    local coords = GetEntityCoords(crop.object)
    local heading = GetEntityHeading(crop.object)

    -- Delete old object
    DeleteObject(crop.object)

    -- Create new object
    local newObj = CreateObject(hash, coords.x, coords.y, coords.z, false, false, false, false, false)
    if not DoesEntityExist(newObj) then
        DBG.Error('SwapPlantModel: failed to create new object for plant: ' .. tostring(plantId))
        return
    end

    SetEntityCollision(newObj, false, false)
    SetEntityCoords(newObj, coords.x, coords.y, coords.z, false, false, false, false)
    SetEntityHeading(newObj, heading)
    FreezeEntityPosition(newObj, true)
    SetEntityCollision(newObj, true, true)

    Crops[plantId].object = newObj
    DBG.Success(('Plant %s swapped to model %s'):format(tostring(plantId), modelName))
end

-- Progress-based growth stage (1–3)
local function UpdateGrowthStage(plantId, plantData)
    local crop = Crops[plantId]
    if not crop or not crop.object or not DoesEntityExist(crop.object) then
        return
    end

    local totalTime = crop.totalTime or plantData.timeToGrow or crop.timeLeft
    local timeLeft = crop.timeLeft or 0
    ...
    if timeLeft > totalTime then timeLeft = totalTime end

    local newStage

    -- 🔑 Stage 3 ONLY when plant is harvestable (timeLeft == 0)
    if timeLeft <= 0 then
        newStage = 3
    else
        -- While growing (>0), only use stage 1 and 2
        local progress = 1.0 - (timeLeft / totalTime)

        -- 0–50% of growth -> stage 1
        -- 50–100% of growth (but still timeLeft > 0) -> stage 2
    ...
    if crop.growthStage == newStage then
        return
    end

    DBG.Info(("[Growth] %s stage -> %s (timeLeft=%d / %d)"):format(
        tostring(plantData.plantName),
        tostring(newStage),
        timeLeft,
        totalTime
    ))
    ...
    -- Optional per-stage model swap
    if plantData.stageProps and plantData.stageProps[newStage] then
        SwapPlantModel(plantId, plantData.stageProps[newStage])
    end
end
...
-------------------------------------------------
-- MAIN PLANT EVENT (matches server)
-------------------------------------------------
-- Server calls:
--  TriggerClientEvent('bcc-farming:PlantPlanted', target, plantId, plantData, plantCoords, timeLeft, watered, src)
...
RegisterNetEvent('bcc-farming:PlantPlanted', function(plantId, plantData, plantCoords, timeLeft, watered, source)
    ...
    local totalTime = tonumber(plantData.timeToGrow) or timeLeft
   ...
   Crops[plantId] = {
    ...
    timeLeft    = timeLeft,
    totalTime   = totalTime,
    growthStage = 0          -- 👈 start at 0, so first call sets stage 1
}

    -- Initialize visual/logical growth stage
    UpdateGrowthStage(plantId, plantData)
    ...
    -- Create thread for time synchronization and growth stages
    CreateThread(function()
        while timeLeft > 0 and Crops[plantId] and not Crops[plantId].removePlant do
            if Crops[plantId].watered == 'true' then
                ...
                if Crops[plantId] then
                    ...
                    -- Update growth stage based on remaining time
                    UpdateGrowthStage(plantId, plantData)
                end
            else
                -- Unwatered: tick faster but don't change timeLeft
                Wait(200)
            end
        end

        -- Ensure final stage when time runs out (if plant still exists)
        if Crops[plantId] and not Crops[plantId].removePlant then
            Crops[plantId].timeLeft = 0
            UpdateGrowthStage(plantId, plantData)
        end
    end)
    ...
    while Crops[plantId] and not Crops[plantId].removePlant do
        ...
        -- 🔹 Global rain watering: if it's raining and plant is dry, water it
        if tostring(crop.watered) == 'false' then
        ...
        if dist <= 1.5 then
            ...
            local currentTimeLeft = crop.timeLeft or 0

            -- 🔹 WATERED plants: countdown / harvest / destroy
            if tostring(crop.watered) ~= 'false' then
                if currentTimeLeft > 0 then
                    ...
                    local noHarvest = _U('plant') .. ': ' .. plantData.plantName ..
                        ' | ' .. _U('secondsUntilharvest') .. string.format('%02d:%02d', minutes, seconds)
                    UiPromptSetActiveGroupThisFrame(
                        HarvestGroup,
                        CreateVarString(10, 'LITERAL_STRING', noHarvest),
                        1, 0, 0, 0
                    )
                else
                    UiPromptSetEnabled(HarvestPrompt, true)
                    ...
                    UiPromptSetActiveGroupThisFrame(
                        HarvestGroup,
                        CreateVarString(10, 'LITERAL_STRING', harvest),
                        1, 0, 0, 0
                    )
                ...
                -- Destroy prompt for WATERED plants
                if Citizen.InvokeNative(0xE0F65F0640EF0617, DestroyPromptHG) then
            ...
            -- 🔹 UNWATERED plants: manual watering + destroy (rain already handled globally)
            if tostring(crop.watered) == 'false' then
                UiPromptSetActiveGroupThisFrame(
                    WaterGroup,
                    CreateVarString(10, 'LITERAL_STRING', _U('waterPlant')),
                    1, 0, 0, 0
                )
                ...
                if Citizen.InvokeNative(0xE0F65F0640EF0617, DestroyPromptWG) then
                    DBG.Info('Destroy prompt completed...')
                    local canDestroy = Core.Callback.TriggerAwait('bcc-farming:HarvestCheck', plantId, plantData, true)
                    if canDestroy then
                        PlayAnim('amb_camp@world_camp_fire@stomp@male_a@wip_base', 'wip_base', 8000, false, true)
