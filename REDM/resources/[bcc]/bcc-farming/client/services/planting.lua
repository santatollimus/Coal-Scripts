...
RegisterNetEvent('bcc-farming:PlantingCrop', function(plantData, bestFertilizer)
    ...
    ---------------------------------------------------------------------------------
	-- Let the player know the raking/planting is done
    Core.NotifyRightTip(_U('plantingDone'), 4000)

    -- Directly plant the crop, skipping fertilizer prompts entirely
    if PlantingProcess then
        DBG.Info('Planting crop at final location (fertilizer prompts disabled)')
        local x, y, z = table.unpack(GetOffsetFromEntityInWorldCoords(playerPed, 0.0, 0.75, 0.0))
