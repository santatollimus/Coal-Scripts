...
RegisterNetEvent('bcc-farming:AddPlant', function(plantData, plantCoords)
    ...
    -- 🔍 Look up full config for this seed from Plants (plants.lua)
    local cfg = nil
    for _, p in pairs(Plants) do
        if p.seedName == plantData.seedName then
            cfg = p
            break
        end
    end
    ...
    if not cfg then
        DBG.Error('No plant config found in Plants table for seedName: ' .. tostring(plantData.seedName))
        -- Fallback: still sync something to the client so it’s not broken
        local targetFallback = Config.plantSetup.lockedToPlanter and src or -1
        TriggerClientEvent('bcc-farming:PlantPlanted', targetFallback, plantId, plantData, plantCoords, plantData.timeToGrow, false, src)
        return
    end

    -- Make sure cfg has a timeToGrow (prefer config, fall back to client data)
    cfg.timeToGrow = cfg.timeToGrow or plantData.timeToGrow

    DBG.Info(('[AddPlant] Created plantId=%s seed=%s timeToGrow=%s'):format(
        tostring(plantId),
        tostring(cfg.seedName),
        tostring(cfg.timeToGrow)
    ))

    -- 👇 Respect lockedToPlanter setting
    local target = Config.plantSetup.lockedToPlanter and src or -1

    -- ✅ Send the full config (including stageProps) to the correct client(s)
    TriggerClientEvent(
        'bcc-farming:PlantPlanted',
        target,
        plantId,
        cfg,                  -- includes stageProps from plants.lua
        plantCoords,
        cfg.timeToGrow,
        false,                -- watered initially false
        src
    )
end)
